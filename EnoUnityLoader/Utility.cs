...
{
    private static bool? _sreEnabled;
    ...
    private static bool CheckSRE()
    {
        try
        {
            if (_sreEnabled.HasValue)
                return _sreEnabled.Value;
    ...
    /// <param name="loader">Function to load the assembly from a path.</param>
    ...
    /// <summary>
    /// Try to resolve and load the given assembly DLL using the ModLoader assembly load context.
    /// </summary>
    /// <param name="assemblyName">Name of the assembly, of the type <see cref="AssemblyName" />.</param>
    /// <param name="directory">Directory to search the assembly from.</param>
    /// <param name="assembly">The loaded assembly.</param>
    ...
    public static bool TryResolveDllAssembly(AssemblyName assemblyName, string directory, out Assembly? assembly) =>
        TryResolveDllAssembly(assemblyName, directory, ModLoaderAssemblyLoadContext.Default.LoadFromPath, out assembly);
    ...
    /// <summary>
    /// Enumerates all methods of a type, including inherited methods.
    /// </summary>
    /// <param name="type">The type to enumerate methods from.</param>
    /// <returns>All methods including inherited ones.</returns>
    ...
    public static string HashStream(Stream stream)
    {
        ...
        return ByteArrayToString(md5.GetHashAndReset());
    }
    ...
    public static string ByteArrayToString(byte[] data) => Convert.ToHexString(data).ToLowerInvariant();
    ...
    public static bool TryParseAssemblyName(string fullName, [NotNullWhen(true)] out AssemblyName? assemblyName)
    {
        try
        {
    ...
    /// <summary>
    /// Adds Cecil platform assemblies to the ModLoader assembly load context for resolution.
    /// Replaces the old AppDomain-based approach.
    /// </summary>
    /// <param name="context">The assembly load context to configure.</param>
    /// <param name="assemblyDir">Directory containing assemblies to add.</param>
    internal static void AddCecilPlatformAssemblies(this ModLoaderAssemblyLoadContext context, string assemblyDir)
    {
        if (!Directory.Exists(assemblyDir))
            return;

        context.AddSearchDirectory(assemblyDir);
    }
