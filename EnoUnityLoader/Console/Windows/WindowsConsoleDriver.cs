...
{
    // Check if running on .NET Core/5+
    private static readonly bool IsCoreCLR = typeof(object).Assembly.GetName().Name == "System.Private.CoreLib";
    ...
    private static readonly ConstructorInfo? FileStreamCtor = new[]
    {
        ...
        AccessTools.Constructor(typeof(FileStream), [typeof(nint), typeof(FileAccess)])
    ...
    private static Stream OpenFileStream(nint handle)
    {
        if (IsCoreCLR)
        {
            ...
            var constructor = AccessTools.Constructor(windowsConsoleStreamType,
                                                      [typeof(nint), typeof(FileAccess), typeof(bool)]);
    ...
    private nint GetOutHandle()
    {
        switch (ConsoleManager.ConfigConsoleOutRedirectType.Value)
        {
            ...
            default:
                return ConsoleWindow.OriginalStdoutHandle != 0
