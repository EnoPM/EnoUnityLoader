...
{
    private readonly ModInfosAttribute? _ownerMetadata;
    ...
    public ConfigFile(string configPath, bool saveOnInit, ModInfosAttribute? ownerMetadata)
    {
    ...
    /// <summary>
    /// Core ModLoader configuration file.
    /// </summary>
    public static ConfigFile CoreConfig { get; } = new(Paths.ModLoaderConfigPath, true);
    ...
    /// <summary>
    /// Access a config entry by section and key.
    /// </summary>
    ...
    public void Reload()
    {
        lock (_ioLock)
        {
            ...
            foreach (var rawLine in File.ReadAllLines(ConfigFilePath))
            {
                ...
                var split = line.Split(['='], 2); //actual config line
                if (split.Length != 2)
    ...
    public void Save()
    {
        lock (_ioLock)
        {
            ...
            var allConfigEntries = Entries
                                   .Select(x => new
                                   {
                                       x.Key, entry = (ConfigEntryBase?)x.Value, value = x.Value.GetSerializedValue()
                                   })
    ...
    /// <summary>
    /// Access one of the existing settings. If the setting has not been added yet, false is returned.
    /// </summary>
    ...
    {
        ArgumentNullException.ThrowIfNull(changedEntryBase);
