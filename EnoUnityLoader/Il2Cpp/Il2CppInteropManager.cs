...
internal static partial class Il2CppInteropManager
{
    ...
    private static readonly ConfigEntry<bool> UpdateInteropAssemblies =
        ConfigFile.CoreConfig.Bind("IL2CPP",
                                   ...
                                   new StringBuilder()
                                       ...
                                       .AppendLine("If disabled assemblies in `ModLoader/interop` won't be updated between game or ModLoader updates!")
                                       .ToString());
    ...
    private static readonly ConfigEntry<string> UnityBaseLibrariesSource = ConfigFile.CoreConfig.Bind(
     ...
     new StringBuilder()
         ...
         .AppendLine("If you want to ensure ModLoader doesn't try to connect to the internet, set this to only the .zip filename (without a URL) and manually place the file in the unity-libs directory.")
         .ToString());
    ...
    private static readonly ConfigEntry<bool> DumpDummyAssemblies = ConfigFile.CoreConfig.Bind(
     ...
     false,
     "If enabled, ModLoader will save dummy assemblies generated by an Cpp2IL dumper into ModLoader/dummy.");
    ...
    private static readonly ConfigEntry<string> IL2CPPInteropAssembliesPath = ConfigFile.CoreConfig.Bind(
     ...
     new StringBuilder()
         ...
         .AppendLine("{ModLoader} - Path to the ModLoader folder.")
    ...
    public static string GameAssemblyPath => Environment.GetEnvironmentVariable("MODLOADER_GAME_ASSEMBLY_PATH") ??
    ...
    private static string IL2CPPBasePath
    {
        get
        {
            ...
            il2cppInteropBasePath = path.Replace("{ModLoader}", Paths.ModLoaderRootPath)
    ...
    private static string RenameMapPath => Path.Combine(Paths.ModLoaderRootPath, "DeobfuscationMap.csv.gz");

    private static ILoggerFactory LoggerFactory { get; } = MSLoggerFactory.Create(b =>
    {
        b.AddProvider(new ModLoaderLoggerProvider())
         .SetMinimumLevel(LogLevel.Trace); // Each ModLoader log listener has its own filtering
    });
    ...
    private static Assembly? ResolveInteropAssemblies(AssemblyLoadContext context, AssemblyName assemblyName)
    {
    ...
    public static void Initialize()
    {
        ...
        AssemblyLoadContext.Default.Resolving += ResolveInteropAssemblies;

        // Add interop assembly path to Cecil search directories for patchers
        TypeLoader.SearchDirectories.Add(IL2CPPInteropAssemblyPath);
    ...
    private static void GenerateInteropAssemblies()
    {
        ...
        try
        {
            ...
            ModLoaderAssemblyLoadContext.Default.AddCecilPlatformAssemblies(UnityBaseLibsDirectory);
            DownloadUnityAssemblies();
            var asmResolverAssemblies = RunCpp2Il();

            if (DumpDummyAssemblies.Value)
            {
                var dummyPath = Path.Combine(Paths.ModLoaderRootPath, "dummy");
    ...
    private static List<AsmResolver.DotNet.AssemblyDefinition> RunCpp2Il()
    {
        var metadataPath = Path.Combine(Paths.GameRootPath,
                                        GlobalMetadataPath.Value
                                                          .Replace("{ModLoader}", Paths.ModLoaderRootPath)
                                                          .Replace("{ProcessName}", Paths.ProcessName)
