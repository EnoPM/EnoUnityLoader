...
{
    /// <summary>
    ///     The main entrypoint of ModLoader, called from Doorstop.
    /// </summary>
    public static void Start()
    {
        // We set it to the current directory first as a fallback, but try to use the same location as the .exe file.
        var silentExceptionLog = Environment.GetEnvironmentVariable("MODLOADER_PRELOADER_LOG") ??
        ...
        try
        {
            ...
            UnityPreloadRunner.PreloaderMain();
        }
        catch (Exception ex)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("Failed to start ModLoader", "EnoUnityLoader");
                }
                else if (NotifySend.IsSupported)
                {
                    NotifySend.Send("Failed to start EnoUnityLoader", "Check logs for details");
                }
                else if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("MODLOADER_FAIL_FAST")))
                {
