...
public static class UnityInfo
{
    ...
    /// </summary>
    /// <remarks>
    ///     Because ModLoader can execute very early, the exact Unity version might not be available in early
    ...
    private static void DetermineVersion()
    {
        ...
        if (PlatformHelper.Is(Platform.Windows))
            try
            {
                var versionInfo = FileVersionInfo.GetVersionInfo(PlayerPath);
                // Parse manually because some games can also wipe the file version (so it's an empty string)
                if (!string.IsNullOrEmpty(versionInfo.FileVersion))
                {
                    var simpleVersion = new Version(versionInfo.FileVersion);
